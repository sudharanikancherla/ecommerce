...
def invoice(ordid):
    if session.get('useremail'):
        ...
        qty=orders[3]
        cost=orders[4]
        cursor.execute('select user_name,address,email from user where email=%s',[username])
        data=cursor.fetchone()
        uname=data[0]
        uaddress=data[1]
        html=render_template('bill.html',uname=uname,uaddress=uaddress,oname=oname,qty=qty,cost=cost,useremail=username,ordid=ordid)
...
def view_contact():
    cursor=mydb.cursor(buffered=True)
    cursor.execute('select * from contactus where email=%s',[name,email,message])
    data=cursor.fetchall()
    cursor.close()
